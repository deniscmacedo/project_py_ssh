...
query_total = """
    select rr."name" as "Regional",ii.inep, ii."name" as "Escola", COUNT (distinct uu.username) as "Total de Estudantes"
    ...
    group by rr."name",ii.inep,ii."name"
""" 
...
query_acessos = """
    ...
    select rr."name" as "Regional",ii.inep, ii."name" as "Escola", COUNT (distinct uu.username) as "Estudantes com Primeiro Acesso"
    FROM sergoias.users_user uu
    left join sergoias.users_user_groups uug on uug.user_id = uu.id 
    left join sergoias.school_classes_schoolclass_users scsu on scsu.user_id = uu.id 
    left join sergoias.school_classes_schoolclass scs on scs.id = scsu.schoolclass_id
...
con.close()
#
result = pd.merge(df_total, df_acessos, on=["Regional","inep","Escola"],suffixes = ("_",None))
result = (
    result
...
#
#
valores_unicos = result['Regional'].unique()

for x in valores_unicos:
  print(x)
  path = f"{output_folder}/visao_{x}.csv"
  parcial = result.query("Regional == @x")
  parcial.to_csv(path, sep=",", encoding="utf-8", index=False, header=True)